...
const testInput = (hiddenNumber, inputData) => {
    ...
    for(let i = 0; i < hiddenNumber.length; i++) {
      if (inputData.includes(hiddenNumber[i]) && hiddenNumber[i] === inputData[i]) {
        bull ++;
    ...
    return {
      inputData: inputData, bull: bull, cow: cow
    }
}
    ...
    <div className="page-wrapper">
      <div className="game-wrapper">
        <div className="header-wrapper">
          <h1>Bulls & Cows!</h1>
          <p>Bulls: {checkBull} Cows: {checkCow}</p>
        </div>
      ...
      <ul>
        {historyInput.map((history, index)=> {
          return <li key={index}>{history.inputData} {history.bull} {history.cow}</li>
        })}
      </ul>
    </div>
  );
